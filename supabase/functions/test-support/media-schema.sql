...
create table public.neighborhood_cluster_members(neighborhood_id uuid,cluster_id uuid,created_at timestamptz default now());
...
create table public.events(id uuid primary key,neighborhood_id uuid,cluster_id uuid,organizer_profile_id uuid,organizer_display_name text,title text,description text,starts_at timestamptz,area_label text,visibility text,status text,moderation_status text,attendee_count integer default 0);
create table public.event_organizers(event_id uuid,profile_id uuid,role text,permissions text[]);
create table public.event_invitations(event_id uuid,inviter_profile_id uuid,invitee_profile_id uuid,status text,expires_at timestamptz,created_at timestamptz default now(),updated_at timestamptz default now());
