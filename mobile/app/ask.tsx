...
export default function AskScreen() {
  ...
  const [question, setQuestion] = useState(typeof params.question === 'string' ? params.question.slice(0, 600) : '');
  ...
  async function ask() {
    if (!neighborhood || busy || question.trim().length < 3) return;
  ...
  return (
    <Screen title="Ask My Corner">
      ...
      <TextInput
        ...
        placeholder={answer ? 'Ask a follow-up…' : 'What’s happening this weekend?'}
        value={question}
      ...
      <Pressable
        ...
        style={styles.button}
      >
        <Text style={styles.body}>{busy ? 'Checking neighborhood sources…' : 'Ask'}</Text>
      </Pressable>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            {(['helpful', 'not_helpful', 'inaccurate'] as const).map((value) => (
              <Pressable key={value} accessibilityRole="button" style={styles.button} onPress={() => void rate(value)}>
                <Text style={styles.body}>
                  {value === 'helpful'
                    ? 'Helpful'
                    : value === 'not_helpful'
