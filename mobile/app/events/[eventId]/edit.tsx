...
export default function EditEventScreen() {
  return <EventsFeatureGate><EditEventContent /></EventsFeatureGate>;
}
...
function EditEventContent() {
  ...
  useEffect(() => {
    if (!eventId) return;
    eventsRuntimeRepository.getEvent(eventId).then((event) => {
      if (!event?.currentUserOrganizerRole) throw new Error('Organizer permission is required.');
      setTitle(event.title);
      setDescription(event.description);
    }).catch((caught) => setError(caught instanceof Error ? caught.message : 'Could not load event.')).finally(() => setLoading(false));
  }, [eventId]);
  ...
  return (
    <Screen title="Edit event" showBottomNavigation={false}>
      {loading ? <Text accessibilityLiveRegion="polite" style={styles.meta}>Loading event...</Text> : null}
      <View style={styles.field}>
        <Text style={styles.label}>Event title</Text>
        <TextInput accessibilityLabel="Event title" editable={!loading} onChangeText={setTitle} style={styles.input} value={title} />
      </View>
      <View style={styles.field}>
        <Text style={styles.label}>Description</Text>
        <TextInput accessibilityLabel="Event description" editable={!loading} multiline onChangeText={setDescription} style={[styles.input, styles.multiline]} value={description} />
      </View>
      {error ? <Text accessibilityRole="alert" style={styles.error}>{error}</Text> : null}
      <View style={styles.actions}>
        <Pressable accessibilityLabel="Cancel editing" accessibilityRole="button" onPress={() => router.back()} style={styles.secondary}><Text style={styles.secondaryText}>Cancel</Text></Pressable>
        <Pressable accessibilityLabel="Save event changes" accessibilityRole="button" accessibilityState={{ disabled: saving || loading }} disabled={saving || loading} onPress={() => void save()} style={styles.primary}><Text style={styles.primaryText}>{saving ? 'Saving...' : 'Save'}</Text></Pressable>
      </View>
    </Screen>
  );
}
