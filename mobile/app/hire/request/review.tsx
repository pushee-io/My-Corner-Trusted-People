...
export default function RequestReviewScreen() {
  ...
  async function submit() {
    const moderation = featureFlags.ai_content_moderation ? await moderateText(params.description ?? '') : { status: 'not_run' as const };
    ...
    trackEvent('request_submitted', { requestId: request.id, categoryId: request.categoryId, moderationStatus: request.moderationStatus });
    router.replace({ pathname: '/hire/request/confirmation', params: { requestId: request.id } });
  }

  return (
    <Screen title="Review request">
      <SuccessState title="Review before sending" body="Check the details. The provider sees your general area, not your exact home address." />
      <View style={styles.panel}>
        ...
        <Text style={styles.notice}>My Corner does not guarantee provider performance. Use the trust evidence, ask questions, and report anything unsafe.</Text>
      </View>
