...
const draft: EventDraft = {
  neighborhoodId: 'east-legon',
  title: 'Neighborhood planning session',
  description: 'Plan the next community cleanup with verified residents.',
  startsAt: '2036-09-12T09:00:00.000Z',
  endsAt: '2036-09-12T10:00:00.000Z',
...
describe('Events complete repository', () => {
  it('creates a moderated draft and rejects unverified creation', async () => {
    ...
    await expect(repository.createEventForViewer(draft, owner)).resolves.toMatchObject({
      status: 'draft',
      moderationStatus: 'pending',
    });
    await expect(
      repository.createEventForViewer(draft, { ...owner, isVerifiedNeighborhoodMember: false }),
    ).rejects.toThrow('Verified neighborhood membership');
  });
  ...
  it('keeps precise locations private until a confirmed RSVP', async () => {
    const repository = createSeededEventsRepository(owner);
    await expect(repository.getEventForViewer('event-east-legon-cleanup', neighbor)).resolves.not.toHaveProperty(
      'preciseLocation',
    );
    ...
    await expect(repository.getEventForViewer('event-east-legon-cleanup', neighbor)).resolves.toMatchObject({
      preciseLocation: 'Private residential address',
    });
  });
  ...
  it('waitlists at capacity and safely promotes no one without an explicit workflow', async () => {
    const repository = createSeededEventsRepository(owner);
    await repository.setGoing('event-east-legon-cleanup', neighbor);
    const result = await repository.setGoing('event-east-legon-cleanup', {
      ...neighbor,
      profileId: 'profile-yaw',
      displayName: 'Yaw T.',
    });
  ...
  it('limits cancellation and organizer management by role', async () => {
    const repository = createSeededEventsRepository(owner);
    await expect(repository.cancelEventForViewer('event-east-legon-cleanup', neighbor)).rejects.toThrow(
      'not authorized',
    );
    await repository.addOrganizer('event-east-legon-cleanup', neighbor.profileId, owner);
    await expect(
      repository.updateEventForViewer('event-east-legon-cleanup', { title: 'Updated title' }, neighbor),
  ...
  it('queues update, cancellation, invitation, and reminder domain events', async () => {
    ...
    await repository.updateEventForViewer(
      'event-east-legon-cleanup',
      { title: 'Cleanup and recycling morning' },
      owner,
    );
    await repository.cancelEventForViewer('event-east-legon-cleanup', owner);
    expect((await repository.listOutbox()).map((item) => item.kind)).toEqual(
      expect.arrayContaining(['event_invitation', 'event_reminder', 'event_updated', 'event_cancelled']),
    );
  });
});
