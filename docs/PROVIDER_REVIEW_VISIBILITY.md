...
Installed APK version 39 does not include this navigation fix. The approved corrected APK is complete: version 40, build workflow `35942254309` and Mobile CI `35942254317` passed for source `c0f019a68c27c9aa71df9b7f51a2fcec11ee55db`. It has the same signing certificate as version 39. See `evidence/provider-review-preview-2026-09-24.json` for download and provenance. The one-build approval is consumed. Device acceptance remains pending: Hire → Kwame → Reviews → inspect stars/title/body/badge/date/author/recommendation/response, then Start request; repeat on Samsung and Pixel Tablet. Record results without calling the checkpoint fully accepted until both devices pass.
...
The founder explicitly approved `20260924005414_provider_review_visibility.sql` on `opeojxwkwwnnncnsuaag` on 2026-09-24, resolving the earlier automatic approval block. Applied successfully as remote version `20260924011448`. An authenticated non-reviewer received average 4.0/count 2/verifiedCount 2, two distinct one-row cursor pages, a terminating final cursor, fictional public author and associated response, and only the approved public projection. Anonymous RPC execution and direct authenticated private-helper execution remain denied. Security advisors reported the existing unrelated baseline categories; neither review API nor the new helper appears among WARN/ERROR findings. Native acceptance remains pending.
